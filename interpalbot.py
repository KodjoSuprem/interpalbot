...
class InterpalsBot():
    ...
    MIN_AGE= 22
    MAX_AGE= 45
    SEX= 'F' #&countries%5B%5D=---&countries%5B%5D=JP&countries%5B%5D=PE
    CONTINENTS= 'EU' # {NA, AS, EU, OC, SA,AF}")
    COUNTRY= ''
    ONLINE=False
    LOGIN= u"myname@gmail.com"  
    ...
    def visitFilteredUsersProfile(self,online=ONLINE,minAge=MIN_AGE,maxAge=MAX_AGE,sex=SEX,continent=CONTINENTS,country=COUNTRY):
        ...
        try:
            ...
            if sex not in self.RequestAttributes["SEX"]:
               ...
               if country != "" and country in self.RequestAttributes["COUNTRY"]:
                   query = self.interpalsAllUsersUrl+"?online=%d&photo=1&age1=%d&age2=%d&sex=%s&sort=last_login&countries=%s"%(1 if online else 2, minAge,maxAge,sex,self.RequestAttributes["COUNTRY"][country])
                   #query = self.interpalsAllUsersUrl+"?photo=1&age1=%d&age2=%d&sex=%s&sort=last_login&countries%5B%5D=---&countries%5B%5D=JP&countries%5B%5D=PE"%(minAge,maxAge,sex,self.RequestAttributes["COUNTRY"][country])
                   print(country)
               else:
                   continentQuery=""
                   print(continent) 
                   for c in continent.split():
                       continentQuery+= "&continents=" + c
                   query = self.interpalsAllUsersUrl+"?online=%d&photo=1&age1=%d&age2=%d&sex=%s&sort=last_login"%( 1 if online else 2, minAge,maxAge,sex) + continentQuery
            ...
            while True:
                ...
                for profile in profiles:
                    ...
                    profilePageResponse = self.makeRequestOrException("GET",url=self.interpalsMainUrl+name.group(0),timeout=10)
                    profileCountry = re.search(r'''<a href="/country/([A-Z]+)" class="nobreak" style="float: left; width: \d+px; margin-top: \d+px;">''',  profilePageResponse.text, re.M)
                    if profileCountry is not None:
                        self.countriesCounter[profileCountry.group(1)] += 1

                    #time.sleep(1)
                    #print(name.group(0))
                    self.nbVisit+=1
                    sys.stdout.write("%d profiles\r" % (self.nbVisit) )
                    sys.stdout.flush()
                     ##if self.nbVisit == 4:
                     ##  return 
                    if self.debugFlag:
                ...
                time.sleep(random.randint(5,70))
...
if __name__ == '__main__':
        try:
            ...
            continent = sys.argv[1] if len(sys.argv) >1 else ''
           
            crawler.visitFilteredUsersProfile( continent= continent)



    #crawler.sendMessage("UserName")
    #crawler.commentProfilePhoto("UserName")        
            print("--done-- " + str(crawler.nbVisit))

            countryMap = crawler.RequestAttributes["COUNTRY"]
            countryMap = {v: k for k, v in countryMap.items()}
            for value, count in crawler.countriesCounter.most_common():
                print(countryMap[value], count)
